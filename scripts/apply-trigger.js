...
const hostMarker = '@db.zayrzorsxgnckauvagtr.supabase.co';
...
const sqlPath = path.join(__dirname, 'update-trigger.sql');
...
async function tryConnectAndRun(pwd) {
  ...
  try {
    ...
    console.log('Executando script SQL para atualizar o trigger...');
    await client.query(sql);
    console.log('Trigger handle_new_profile_details atualizado com sucesso!');
    await client.end();
    return true;
  } catch (err) {
    console.log(`Falha ao conectar usando a senha "${pwd}":`, err.message);
...
async function run() {
  // Testar com colchetes: "[Ca09091976@!#]"
  let success = await tryConnectAndRun(parsedPassword);
  if (success) return;

  // Testar sem colchetes: "Ca09091976@!#"
  ...
  console.error('Não foi possível conectar ao banco de dados com nenhuma das variações da senha.');
}
